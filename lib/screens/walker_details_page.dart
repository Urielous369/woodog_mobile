...
class WalkerDetailsPage extends StatefulWidget {
  Walker walkerView;
  WalkerDetailsPage({super.key, required this.walkerView});

  @override
  State<WalkerDetailsPage> createState() => _WalkerDetailsPageState();
}

class _WalkerDetailsPageState extends State<WalkerDetailsPage> {
  ...
  //  isRecruited = widget.walkerView.isRecruited ?? false;
   ...
   Future<void> checkRecruitmentStatus() async {
    bool recruitedStatus = await checkIfIsRecruited(widget.walkerView.id!);
  ...
  Future<void> _recruitWalker() async {
    ...
    ApiResponse response = await recruitWalker(widget.walkerView.id!);
            ...
            child: Container(
              decoration: BoxDecoration(
                image: DecorationImage(
                  image: NetworkImage(widget.walkerView.photo_url ?? ''),
                  fit: BoxFit.cover,
                ),
              ),
            ),
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            '${widget.walkerView.monthOfExperience} months',
                            style: const TextStyle(
                                fontSize: 16, fontWeight: FontWeight.bold),
                          ),
                        ],
